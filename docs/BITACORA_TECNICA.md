...
| [H26](#h26) | Ops | Drift repo/prod: `vector.production.toml` real en `.139` no está bajo git, difiere sustancialmente del tracked en `develop` | Medio | Resuelto en `.139`: reconciliado, `.139` convertido a checkout real de git con deploy key de solo lectura, `vector-soar.service` reapuntado y validado con tráfico real. `.140` pendiente de la misma conversión (excluyendo `motor/model.py`, ver H31) |
...
**`.140` — pendiente de la misma conversión**, con una restricción explícita: `motor/model.py` (H31) debe quedar excluido de la sincronización automática (`git update-index --skip-worktree`) antes de que cualquier `git pull` pueda tocar ese directorio, para no sobreescribir la versión real de producción con la del repo, hoy desactualizada.
...
- **Mecanismo de saturación de `motor-soc.service` identificado, disparador inicial no confirmado** (H30): IO síncrono (Redis sin timeout en `response/queue.py`) y CPU-bound (`model.predict()`) ejecutados directamente dentro de `async def decide()`, sin `--workers`, sin paralelismo real. Pendiente decidir con Antonio si se corrige (threadpool para el modelo, timeout en el Redis de `queue.py`, evaluar `--workers 2`) antes de que vuelva a ocurrir, y agregar monitoreo de CPU/latencia de este servicio específico (mismo patrón pendiente de H22/H24/H25 para otros servicios).
