...
| [H26](#h26) | Ops | Drift repo/prod: `vector.production.toml` real en `.139` no está bajo git, difiere sustancialmente del tracked en `develop` | Medio | Documentado — no reconciliado, opciones de solución propuestas, decisión pendiente |
...
- **`motor-soc.service` con saturación de CPU recurrente, causa raíz no investigada** (H29): proceso `uvicorn` corriendo sin `--workers` (un solo proceso) desde el 3-sep; se satura a 400%+ CPU con cola de conexiones creciente cada ~20 minutos tras un restart de servicio simple. Un reboot completo del servidor lo resolvió de forma estable el 2026-09-05, pero no se investigó por qué un solo proceso llega a ese nivel de CPU (candidatos: volumen real de tráfico incluyendo el ruido interno documentado en H28, contención de recursos con otro proceso del host, o costo de inferencia del modelo bajo carga sostenida sin perfilar). Evaluar correr con `--workers 2` (dentro del límite ya documentado en CLAUDE.md) y/o agregar el mismo tipo de monitoreo pendiente de H22/H24/H25 (health-check que además mida latencia/CPU, no solo `active`).
