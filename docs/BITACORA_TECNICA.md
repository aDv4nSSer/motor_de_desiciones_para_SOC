...
| [H23](#h23) | Resp | OTX/AlienVault como segunda fuente de R1 (ampliación SOAR, punto 1) | Medio | Implementado y desplegado en `.140` — validado con evento real (ver H24). Pendiente de carga cerrado con evidencia real: 0 `429` en 15 lookups reales, pero volumen real de ataque sigue sin poder medirse hasta resolver H28 (IP privada); cuota real de la cuenta sigue sin confirmar oficialmente (fuentes contradictorias) |
...
| [H27](#h27) | Ops | Credenciales de OpenSearch desincronizadas en 4 lugares (`opensearch-indexer.service`, `dashboard.py`, `vector-soar.env`, `shadow-detect.env`) por falta de `load_dotenv()` y nombres de variable inconsistentes | Alto | **CERRADO** — password real rotada (`internal_users.yml` + `securityadmin.sh` vía puerto REST), los 4 consumidores actualizados y validados con evidencia real (conteos creciendo, healthcheck, antes/después de `shadow-detect`) |
| [H28](#h28) | Resp | Corroboración multi-fuente de R1 (AbuseIPDB+OTX) no influía en tier/`accion_recomendada` — solo quedaba en `soc:response:audit` sin efecto en R2; además, R1 en producción enriquece IPs privadas post-migración VLAN (`10.10.10.3`/`10.30.30.2`), lo que rompía OTX (HTTP 400) y explica por qué la corroboración real nunca se validó bajo carga | Alto | Resuelto y validado en producción: gate real en `worker.py` (2+ fuentes → automático, <2 → `BLOCK_PENDING_APPROVAL` N1) + guard de IP privada en `enrichment.py`, desplegado a `.140` y confirmado con evento real (`worker.log`). 25 tests nuevos, 44/44 pasan. Circuit-breaker evaluado y descartado con evidencia. Efecto colateral medido: guard también drenó un backlog real de 99.594 mensajes. Causa raíz del "por qué casi todo es IP privada" identificada en H34 |
| [H34](#h34) | Red | El tráfico de ataque externo real contra `.138`/`.139` nunca llega a Suricata — la cola NFQUEUE (`-q 0`) solo existe en la cadena `ufw-before-forward`, no en INPUT/OUTPUT, dejando invisibles el redirect de Cowrie y el reverse proxy hacia `.138` | Alto | Causa raíz identificada y verificada paso a paso (0/5 conexiones confirmadas de Cowrie, 0 eventos HTTP hacia `.138:80` en 20k líneas de `eve.json`). Efecto colateral no buscado de H21. **No se aplicó ningún fix** — es una decisión de topología de red que le corresponde a Antonio, 3 opciones presentadas |
...
## H27 (cierre) — rotación real de la password de OpenSearch, los 4 consumidores validados

**Fecha:** 2026-09-05

**Contexto:** la password real de OpenSearch quedó expuesta en una sesión de trabajo anterior (ver H27 original) y nunca se rotó — quedaba pendiente como "tarea separada y deliberada" por el riesgo de dejar el cluster sin autenticación funcional si algo salía mal (single-node, sin réplicas, guarda el hash-chain de `soc-decisions`).

**Backup previo:** `tar.gz` completo de `/usr/share/opensearch/config/opensearch-security/` (todo el directorio de configuración de seguridad, no solo `internal_users.yml`) copiado fuera del contenedor a `.140` antes de tocar nada. No se armó infraestructura de snapshot de datos (`path.repo` + snapshot API) porque lo que estaba en riesgo era la config de seguridad, no los datos de los índices — la rotación de un usuario no toca ningún dato de `soc-decisions`.

**Rotación:** password nueva generada con `secrets.token_urlsafe(24)` en `.140`, nunca impresa. Hasheada con la propia herramienta de OpenSearch (`hash.sh -env <VAR>`, nunca `-p <password>` en texto plano por línea de comandos — evita que aparezca en `ps aux` de otros usuarios del host). `internal_users.yml` actualizado con el nuevo hash (reemplazo verificado, exactamente 1 ocurrencia) y aplicado con `securityadmin.sh` contra el puerto REST (`9200`, no `9300` — la primera prueba con el puerto de transporte falló con "This is not an HTTP port"; las versiones actuales de `securityadmin.sh` hablan REST, no protocolo de transporte binario). `Done with success`.

**Validado inmediatamente:** password nueva autentica (`200`), intento con una password arbitraria de prueba falla (`401`) — confirma que el auth real está activo, no solo que el comando no dio error.

**4 consumidores actualizados y validados, no solo 1:**
1. `~/tesis/motor-runtime/.env` (`.140`, usado por `opensearch_indexer.py` y por `dashboard.py` vía `load_dotenv()`) — `opensearch-indexer.service` reiniciado, conteo de `soc-decisions` confirmado creciendo en tiempo real después (4.008.721 → 4.008.728).
2. `/etc/motor-soc/dashboard.env` (`.140`, `EnvironmentFile` de `motor-soc.service`) — `motor-soc.service` reiniciado, `/health` sano.
3. `/etc/vector/vector-soar.env` (`.139`, sink `suricata_alerts_os`) — `vector-soar.service` reiniciado, `Healthcheck passed` en el log de arranque (Vector valida conectividad+auth de sus sinks al iniciar). No se pudo forzar una alerta real de Suricata en el momento para una prueba end-to-end con documento nuevo — limitación reconocida, no se infló la evidencia disponible.
4. `/etc/motor-soc/shadow-detect.env` (`.139`, timer `shadow-detect.timer`) — corrida forzada con `systemctl start`: pasó de `ERROR OpenSearch HTTP error` (15:43, password vieja ya inválida en el servidor, archivo todavía sin actualizar) a `4 hallazgos escritos` (15:48, justo después de actualizar el archivo) — el antes/después más claro de los cuatro.

**Evidencia:** ver detalle de cada consumidor arriba; backups con timestamp de cada archivo tocado (`internal_users.yml`, `.env`, `dashboard.env`, `vector-soar.env`, `shadow-detect.env`) conservados junto a cada original. Ningún archivo de secretos se commiteó al repo — todo el trabajo fue en los hosts.

**Estado: CERRADO.** No queda ninguna rotación de OpenSearch pendiente.

---

## H32 (cierre) — rotación real de `REDIS_PASSWORD`, con un incidente de exposición propio en el camino

**Fecha:** 2026-09-05

**Password nueva generada** con `secrets.token_urlsafe(24)` en `.140`, nunca impresa en su forma final (ver incidente abajo sobre un intento anterior que sí se expuso).

**Incidente de exposición durante la rotación — reportado de inmediato, no minimizado:** el primer intento de aplicar el cambio en el Redis real usó `redis-cli CONFIG SET requirepass <nueva>`, que falló porque este Redis tiene `CONFIG` renombrado a un comando alternativo secreto (`rename-command CONFIG "CONFIG_SOC_ADMIN_9x7k"` en `/etc/redis/redis.conf` — medida de hardening ya existente, junto con `FLUSHALL`/`FLUSHDB`/`DEBUG`/`SHUTDOWN` deshabilitados por completo). El mensaje de error de `redis-cli` devolvió el comando completo, **incluida la password nueva en texto plano**, y ese output se imprimió sin filtrar — quedó expuesto en esta conversación. Se trató de inmediato como comprometido: se descartó esa password (nunca se llegó a aplicar al servidor real ni a ningún consumidor, verificado) y se generó una segunda password nueva desde cero. El script de rotación se reescribió para no imprimir jamás stdout/stderr crudo de `redis-cli`, solo un resumen `OK`/`FALLO`.

**Rotación aplicada (segunda password, la que quedó vigente):** `requirepass` está en `/etc/redis/soc-motor.conf` (incluido desde `redis.conf` como último `include`, no en el archivo principal — hallazgo intermedio, el primer intento apuntó al archivo equivocado y falló de forma segura sin tocar nada). Aplicado con el comando `CONFIG` renombrado, `CONFIG SET` confirmado exitoso, `PING` con la password nueva exitoso, `PING` con la vieja fallido — los tres resultados impresos como texto plano `OK`/`FALLO`, nunca el detalle crudo del comando.

**3 consumidores de `.140` (los pedidos explícitamente) actualizados y validados:**
1. `~/tesis/motor-runtime/.env` → `motor-soc.service` reiniciado, `/decide` real con modelo `golden4_v7_1`.
2. Mismo `.env` → `response-worker.service` reiniciado (no está en el `NOPASSWD` de auditoría, requirió a Antonio), `worker.log` con líneas R1 nuevas sin errores de Redis.
3. Mismo `.env` → `opensearch-indexer.service` reiniciado, conteo de `soc-decisions` confirmado creciendo (4.025.103 → 4.025.107).

**Evidencia:** backups con timestamp de `.env` y `soc-motor.conf` antes de cada cambio. Sin ninguna password (ni la quemada por el incidente, ni la vigente) commiteada al repo, impresa en un commit, o guardada en ningún archivo de este repositorio.

**Estado: CERRADO** para los 3 consumidores de `.140` explícitamente pedidos. Ver H33 para un hallazgo relacionado, no resuelto, descubierto al intentar validar consumidores adicionales en `.139` que no estaban en el alcance original de esta tarea.

---

<a id="h33"></a>
## H33 — `vigilante/cases.py` (heartbeat + casos del FIM en `.139`) nunca pudo alcanzar Redis en `.140`, con o sin la rotación de hoy

**Fecha:** 2026-09-05

**Contexto:** al intentar validar, más allá de lo pedido explícitamente, si los consumidores de Redis en `.139` (`vigilante/cases.py`, usado por `motor-watcher.service` y `motor-watcher-heartbeat.service`) seguían sanos tras la rotación de H32, se encontraron dos problemas encadenados, ninguno causado por la rotación de hoy.

**Hallazgo 1 — `REDIS_HOST` nunca estuvo configurado en `/etc/motor-soc/watcher.env`:** `cases.py` usa `os.environ.get("REDIS_HOST", "200.54.12.140")` — el mismo patrón de fallback a la IP pública obsoleta que causó el apagón de 17 días de H25. Al forzar una corrida de `motor-watcher-heartbeat.service`, falló con `Error 113: No route to host` contra `200.54.12.140:6379` — confirmando que el fallback obsoleto es lo que efectivamente se usa, no un valor correcto configurado en alguna parte. Corregido agregando `REDIS_HOST=10.10.10.3` a `watcher.env` (con backup del archivo).

**Hallazgo 2 — con el host correcto, la conexión se rechaza igual:** tras el fix del host, la misma corrida forzada falló con `Error 111: Connection refused` contra `10.10.10.3:6379` — no "no route", sino rechazo activo. Indica que Redis en `.140` probablemente solo acepta conexiones en `127.0.0.1` (bind a loopback), y nunca fue configurado para aceptar conexiones desde otros hosts de la red, incluido `.139`. **No se investigó ni se tocó la configuración de bind de Redis** — abrir Redis a conexiones de red es una decisión de seguridad (superficie de ataque de un servicio que hoy además almacena la cola de R1/R2) que le corresponde a Antonio, no algo para resolver unilateralmente en el camino de una rotación de credenciales.

**Alcance real del problema:** `vigilante/cases.py::write_heartbeat()` se llama desde `watcher.py` dentro de un `try/except` (no crashea el servicio), así que el fallo es silencioso — coincide con el patrón de "nadie se entera hasta que alguien audita a mano" ya visto en H22/H24/H25/H27. Esto significa que el mecanismo de heartbeat diseñado específicamente para detectar "el vigilante FIM murió en silencio" **nunca pudo funcionar**, porque el propio heartbeat nunca pudo escribirse. No se determinó desde cuándo (podría ser desde la migración de VLAN de agosto, o desde siempre) — no hay evidencia suficiente en esta sesión para fecharlo con precisión.

**Decisión:** no se revierte el fix de `REDIS_HOST` (es estrictamente más correcto que el fallback obsoleto, aunque hoy no alcance para que la conexión funcione). No se intenta abrir Redis a la red sin decisión explícita de Antonio. Password de Redis en `watcher.env` queda igual actualizada a la nueva (correcta, aunque hoy no se pueda validar por el problema de bind).

**Pendiente real:** decidir si `.139` necesita alcanzar Redis en `.140` de verdad (y si es así, con qué restricción de red/firewall — no bind abierto a cualquier IP) o si `cases.py`/`heartbeat_check.py` deberían rediseñarse para no depender de una conexión directa cross-host a Redis.
...
**Contexto:** pendiente desde H23 original ("validar en `.140` con volumen de tráfico real sostenido que la cuota/rate limit de OTX es compatible — no se ha medido bajo carga"). Nunca se había hecho, solo probado con eventos puntuales.

**Paso 1 — ¿la API expone su cuota directamente?** Se probó pedir un indicador real y revisar headers de respuesta buscando `X-RateLimit-*`/`Retry-After` u otro mecanismo estándar de anuncio de cuota. **OTX no expone esta información en ningún header** — a diferencia de AbuseIPDB, que sí lo hace. Esto obliga a inferir el límite empíricamente o por documentación externa, no se puede leer directo de la API.

**Paso 2 — la cifra de cuota real es genuinamente contradictoria en fuentes públicas:** una búsqueda encontró "~10.000 req/hora con API key" (reportes de comunidad AT&T/LevelBlue); otra búsqueda, en la misma sesión, encontró "tier gratuito: 100/mes" (listado de un directorio de APIs de terceros). La diferencia entre ambas cifras es de más de tres órdenes de magnitud — no se resolvió cuál es correcta, ninguna se pudo confirmar contra documentación oficial vigente (las páginas oficiales de OTX son una SPA sin contenido accesible por fetch simple).

**Paso 3 — decisión: prueba de carga acotada y conservadora, no un intento de encontrar el límite exacto.** Dado que una cifra plausible es tan baja como 100/mes, escalar agresivamente para encontrar el punto exacto de `429` arriesgaba consumir una cuota mensual ajustada en un solo intento de esta sesión — un costo real y desproporcionado frente al valor de conocer el número exacto. Se optó por una prueba pequeña (15 IPs públicas reales y distintas — resolvers DNS/CDN conocidos, para evitar caché local y garantizar que cada llamada golpeara la API real) y no escalar más si no aparecía señal de throttling.

**Resultado de la prueba:** 15/15 requests reales, ninguna con `429`. 9/15 exitosas (`200`), 6/15 con `ReadTimeout` (con el timeout de prueba de 6s, más generoso que el `otx_timeout=4.0` real de producción — en producción estas 6 hubieran fallado más rápido, marcado `otx_available=False`, y no contado a favor ni en contra de la corroboración de H28, exactamente el diseño ya validado en H24). **Hallazgo colateral, no es prueba de rate-limit pero sí de confiabilidad:** una tasa de `ReadTimeout` del 40% en un lote modesto es un dato real a tener en cuenta — no se investigó su causa (¿latencia geográfica, sobrecarga del lado de OTX, o limitación blanda sin `429` explícito?), queda como observación, no como hallazgo cerrado.

**Paso 4 — volumen real que el sistema genera hoy, con número concreto (no "parece que aguanta"):** lectura de `soc:response:audit` (ventana real de 11.23 horas, 100.001 registros): **10 registros con IP pública** (0.01% del total), correspondientes a solo **3 IPs públicas distintas** en toda la ventana — la mayoría, si no todas, originadas por las propias pruebas de esta sesión (H28-H30), no por tráfico de ataque real. Eso da **0.267 IPs públicas distintas por hora** como proxy del volumen real de lookups OTX que el sistema genera hoy (con caché de 6h, el consumo de cuota depende de IPs distintas, no de eventos totales).

**Margen real — respuesta condicionada a la cifra de cuota, honestamente, no una sola cifra inventada:**
- Si la cuota real es ~10.000/hora: margen de **>99.99%** — el sistema hoy no se acerca ni de lejos.
- Si la cuota real es ~100/mes (≈0.139/hora en promedio): el volumen actual (0.267/hora, casi todo tráfico de prueba propio) ya está en el mismo orden de magnitud que ese promedio — **no hay margen demostrable bajo esta hipótesis**, y una vez que se resuelva el bug de IP privada de H28 y empiecen a llegar IPs de atacantes externos reales, el consumo podría superar un presupuesto mensual así de ajustado en cuestión de días.

**Esto NO se puede cerrar del todo hoy, y se dice explícitamente:** el volumen real de IPs públicas distintas por hora que el sistema generaría bajo tráfico de ataque genuino sigue sin poder medirse, porque el bug de IP privada de H28 sigue sin resolver — casi todo el tráfico que llega a R1 hoy es ruido interno, no ataques externos. La cifra de 0.267/hora de esta sesión **no representa carga real del sistema**, representa la ausencia de carga real (más el ruido de mis propias pruebas).

**Recomendación, no ejecutada hoy (fuera de lo que se puede hacer con la API key sola):** el dato definitivo — el tier y la cuota real contratada de esta cuenta de OTX — está disponible con certeza en el panel de la cuenta de OTX (login web), no reconstruible con certeza desde la API. Antonio puede confirmarlo en un par de minutos revisando el dashboard de su cuenta OTX, lo cual resuelve la ambigüedad de fuentes de manera más barata y confiable que seguir escalando pruebas contra la API real.

**Decisión sobre si el cache/degradación actual alcanza:** para el volumen de HOY, sobra por completo (la demanda real es casi nula). Para el volumen que se generaría con tráfico de ataque real, **no se puede afirmar que alcance ni que no alcance** hasta que H28 se resuelva y haya datos reales que medir — se documenta como dependencia explícita entre ambos pendientes, no se promete una respuesta que no se tiene.

**Evidencia:** script de prueba (15 IPs públicas reales, códigos de respuesta y tiempos por cada una); lectura de `soc:response:audit` con cálculo de IPs públicas distintas/hora; búsquedas web citadas con sus fuentes (ninguna confirmada contra documentación oficial).

---

<a id="h34"></a>
## H34 — El tráfico de ataque externo real contra `.138`/`.139` nunca llega a Suricata: la cola NFQUEUE solo existe en la cadena FORWARD, no en INPUT/OUTPUT

**Fecha:** 2026-09-05

**Severidad:** Alto — probablemente el hallazgo de mayor impacto para la validez de las métricas de R1/R2 de toda la tesis, después de H31. Explica, con una causa raíz concreta, por qué H28 encontró que casi todo el tráfico que llega al motor es ruido interno.

**Contexto:** cierre de H28 dejó anotado "`src_ip` llegando como IP privada al Fast Path" como problema de red/Suricata/Vector, diferido sin diagnosticar. Con la corroboración multi-fuente de H28 y la validación de OTX de H23 mostrando solo 3 IPs públicas distintas en 11+ horas (mayormente de pruebas propias), esto pasó a ser prioridad: sin resolverlo, ninguna métrica de efectividad de R1/R2 es confiable para la tesis.

**PASO 0 — diagnóstico con evidencia real, sin asumir dónde estaba el problema:**

**Fuente de verdad 1 — honeypot Cowrie en `.139` (por diseño, solo recibe tráfico externo):** se leyeron las conexiones reales más recientes de `cowrie.json` — 5 IPs públicas confirmadas como externas, todas contra el puerto 2223 (redirect de SSH): `2.57.122.150`, `185.93.89.23`, `91.236.158.174`, `2.57.121.112`, `113.201.21.110`. Se buscaron esas mismas 5 IPs en `eve.json` de Suricata, en cualquier campo, no solo `src_ip`: **0/5 aparecen en absoluto** — ni con la IP correcta, ni con una incorrecta. No es un problema de "IP equivocada", es que el tráfico nunca llega a Suricata.

**Fuente de verdad 2 — servidor web real en `.138` (contenedor `demo-landing`, puerto 80 públicamente expuesto):** los logs del propio nginx dentro del contenedor muestran tráfico de escaneo real y continuo (patrón `POST /WebServices/Device` — escaneo IoT/UPnP típico de ruido de internet), pero con `client: 192.168.153.x` — un rango que no coincide con ninguna VLAN del proyecto (10.10.10.0/24, 10.20.20.0/24, 10.30.30.0/24), consistente con el bridge interno de Docker. Se buscó en 20.000 líneas recientes de `eve.json` cualquier evento con `dest_ip=10.30.30.2` y `dest_port=80` (el tráfico HTTP entrante real): **0 resultados**, pese al tráfico constante y confirmado por el propio nginx.

**Diagnóstico de la causa raíz (no se asumió, se verificó paso a paso):**
1. `af-packet: interface eno2` aparece en `suricata.yaml`, pero **no es el mecanismo activo** — `ps aux`/`systemctl show` confirman que Suricata corre con `-q 0` (modo NFQUEUE inline), no af-packet. La sección af-packet del yaml es config no usada, un resabio.
2. Con NFQUEUE, DNAT/REDIRECT (aplicados en `PREROUTING`) **no reescriben la IP de origen** — solo la reescribe SNAT/MASQUERADE (aplicado en `POSTROUTING`, para tráfico saliente). Esto descartó la hipótesis de que el problema fuera de reescritura de IP por NAT.
3. `iptables -t nat -L -n -v`: la única regla de PREROUTING relevante es `REDIRECT tcp dpt:22 redir ports 2223` (el redirect de Cowrie) — **no existe ninguna regla de DNAT hacia `.138:80`** en la tabla `nat` de `.139`.
4. `/etc/nginx/sites-available/reverse-proxy-443` en `.139` (hallazgo no buscado, encontrado en el camino): expone `demo-landing` (y el dashboard del motor) vía **reverse proxy de aplicación** (`proxy_pass http://200.54.12.138:80` — usa además la IP pública obsoleta de `.138`, mismo patrón de H25/H26, no investigado a fondo por no ser el foco de esta tarea), con `X-Real-IP`/`X-Forwarded-For` correctamente configurados — pero esa información nunca llega a Suricata ni a Vector, se queda en el log de nginx.
5. `iptables -S | grep -B5 NFQUEUE`: la única regla `-j NFQUEUE --queue-num 0` está en la cadena **`ufw-before-forward`**. Se verificó explícitamente, listando `ufw-before-input` y `ufw-before-output` completas: **ninguna de las dos tiene una regla NFQUEUE**.

**La causa raíz, en una frase:** Suricata solo inspecciona tráfico **FORWARDED** (enrutado a través de `.139` entre otros dos hosts). No inspecciona tráfico **destinado a `.139` mismo** (cadena INPUT — el caso de Cowrie, cuyo REDIRECT convierte el paquete en "entrega local" antes de que pueda llegar a FORWARD) ni tráfico **originado por `.139` mismo** (cadena OUTPUT — el caso del reverse proxy, que abre una conexión nueva desde `.139` hacia `.138`). Ambos casos, que son exactamente los dos mecanismos reales por los que el tráfico externo llega a los servicios protegidos, caen fuera de la única cadena donde existe la desviación a Suricata.

**Relación con H21, verificada, no asumida:** H21 (25-ago) cambió a Suricata de captura pasiva AF-PACKET a modo in-line real vía NFQUEUE, específicamente para resolver "cero alertas". La validación de H21 usó tráfico de prueba explícitamente descrito como "contra un host detrás del gateway" — es decir, tráfico FORWARDED. Nunca se probó tráfico destinado a `.139` mismo ni originado por `.139` mismo, así que H21 no tenía cómo revelar este hueco. **Es un efecto colateral no buscado del propio fix de H21**: antes de H21, AF-PACKET pasivo capturaba cualquier paquete en el cable sin importar a qué cadena de netfilter iba destinado — el cambio a NFQUEUE, necesario para que Suricata viera el tráfico completo de cada conexión (no solo el primer paquete), acotó sin que nadie lo decidiera explícitamente la inspección a una sola cadena.

**Cuantificación pedida (PASO 0.4), sin promediar ni suavizar:** de 5 conexiones confirmadas como externas por Cowrie, **0 (0%)** llegaron a Suricata con cualquier IP (correcta o incorrecta). De todo el tráfico HTTP real y continuo hacia `demo-landing` confirmado por el log de nginx, **0 eventos** aparecen en Suricata para ese destino/puerto en absoluto. No es un problema de exactitud del dato — es ausencia total del dato.

**Esto explica directamente el hallazgo de H28:** no es que el motor reciba mayormente ruido interno *en vez de* ataques externos correctamente atribuidos — es que los ataques externos reales contra los servicios que el proyecto dice proteger (`.138` vía el reverse proxy, el redirect de Cowrie en `.139`) nunca llegan a Suricata, nunca llegan a Vector, nunca llegan al motor, en ninguna forma. El ruido interno que sí ve el motor (DNS de `.140`/`.138` hacia servidores públicos) es tráfico *forwarded* genuino — por eso ese sí se ve, y se ve con el `src_ip` correcto (esos flujos no tienen ningún problema de atribución, son ruido real correctamente atribuido).

**Rango de fechas contaminado:** el mecanismo (NFQUEUE solo en FORWARD) existe desde el fix de H21 (25-ago-2026) hasta hoy — anterior a eso, Suricata en AF-PACKET pasivo no tenía esta limitación específica (aunque sí el problema distinto que H21 resolvió). Combinado con la ventana ya excluida de H25 (18-ago a 04-sep, apagón total de ingestión), el rango con datos reales pero con este blind spot activo es **04-sep-2026 en adelante** — es decir, prácticamente toda la ventana de datos "reales" que existe hoy tiene este problema. No hay ventana de datos limpia de este efecto todavía.

**PASO 1 — decisión de tocar la topología: NO aplicada, es de Antonio.** Esto es exactamente el caso de limitación estructural que se pidió no resolver unilateralmente. Opciones identificadas, sin aplicar ninguna:
1. **Agregar la misma desviación NFQUEUE a `ufw-before-input` y `ufw-before-output`.** Le daría a Suricata visibilidad sobre tráfico hacia/desde `.139` mismo, incluyendo el redirect de Cowrie y el reverse proxy. Riesgo: aumenta la carga de inspección de Suricata a incluir todo el tráfico propio de `.139` (SSH de administración, tráfico de monitoreo, DNS del propio host) — no medido cuánto more volumen implica. Requiere repetir la validación de H21 (orden de reglas fast-path vs. NFQUEUE) para las cadenas nuevas, no asumir que el mismo orden ya es correcto ahí.
2. **Corregir el reverse proxy para pasar `X-Forwarded-For` y hacer que Vector lea el log de nginx directamente (una fuente de ingesta nueva y separada de `eve.json`) para el tráfico HTTP proxied.** Resuelve el caso del reverse proxy específicamente, sin tocar la política de NFQUEUE. No resuelve el caso de Cowrie (aunque Cowrie, por diseño, está deliberadamente aislado de la tubería principal — puede que ese caso ni necesite resolverse).
3. **No tocar nada y documentar la limitación como alcance de la tesis.** Honesto, pero significa que ninguna métrica de "efectividad de R1/R2 contra ataques reales" puede sostenerse con los datos actuales — solo con tráfico interno (que no es lo que R1/R2 está diseñado para juzgar).

Ninguna opción es aplicada. **PASO 2 y 3 (validar el fix, re-medir el rate limit de OTX con tráfico real) quedan bloqueados hasta que Antonio decida.**

**Evidencia:** conexiones de Cowrie (`cowrie.json`, 5 IPs con timestamp); logs de `demo-landing` (docker logs, WSDAPI scanning real); búsqueda de esas IPs y de `dest_ip=10.30.30.2 dest_port=80` en `eve.json` (0 resultados en ambos casos); `ps aux`/`systemctl show suricata` confirmando modo `-q 0`; `iptables -t nat -L -n -v`; `/etc/nginx/sites-available/reverse-proxy-443`; `iptables -S | grep NFQUEUE` y verificación directa de `ufw-before-input`/`ufw-before-output` sin la regla.
...
- **Tasa de `ReadTimeout` del 40% de OTX en una prueba de 15 requests** (H23): no se investigó la causa (latencia geográfica, sobrecarga del proveedor, o throttling blando sin `429` explícito). El timeout de producción (4s) y la degradación elegante ya manejan esto correctamente, pero la causa de fondo de una tasa de fallo tan alta en una muestra chica no se entendió.
